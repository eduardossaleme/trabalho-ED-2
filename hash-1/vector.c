...
int vector_find(Vector *v, data_type val, CompareFunction cmp_fn){
    for(int i=0; i<v->size; i++){
        if(cmp_fn(val, v->data[i])==0) return i;
    }
    return -1;
}
...
int vector_argmax(Vector *v, CompareFunction cmp_fn){
    return vector_find(v, vector_max(v, cmp_fn), cmp_fn);
}

int vector_argmin(Vector *v, CompareFunction cmp_fn){
    return vector_find(v, vector_min(v, cmp_fn), cmp_fn);
}
...
int vector_binary_search(Vector *v, data_type val, CompareFunction cmp_fn){
    int max, min, med;
    min=0;
    max=v->size-1;
    while(max<v->size){
        med=(max+min)/2;
        if(cmp_fn(v->data[med], val)==0){
            return med;
        }
        else if(cmp_fn(v->data[med], val)<0){
            min=med+1;
        }
        else{
            max=med-1;
